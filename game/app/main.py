...
async def play_page(request: Request):
    # Если в браузере уже активна сессия мастера игры — не пускаем в кабинет
    # игрока. Это закрывает сценарий «ГМ запускает игру, жмёт «назад» и
    # подключается к своей же сессии как обычный игрок».
    if is_gm_cookie_valid(request.cookies.get(SESSION_COOKIE)):
        return RedirectResponse(
            url="/admin?reason=gm-session-active",
            status_code=303,
        )
