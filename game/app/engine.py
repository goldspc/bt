...
# Делаем корневую папку проекта (`game/`) импортируемой, чтобы можно было
# переиспользовать `shared_simple`, `protocol`, `server_full_visibility`
# без установки пакета. __file__ = game/app/engine.py → два dirname даёт game/.
